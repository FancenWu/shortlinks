...
public interface LinkBrowserStatsMapper extends BaseMapper<LinkBrowserStatsDO> {
     ...
     * 记录浏览器访问监控数据
     */
    @Insert("insert into t_link_browser_stats (gid, full_short_url, date, cnt, browser, create_time, update_time, del_flag) " +
            "VALUES (#{linkBrowserStats.gid}, #{linkBrowserStats.fullShortUrl}, #{linkBrowserStats.date}, #{linkBrowserStats.cnt}, #{linkBrowserStats.browser}, now(), now(), 0) ON DUPLICATE KEY " +
